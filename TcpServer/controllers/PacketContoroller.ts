...
Description : 페킷 데이터 버퍼에 저장 및 컨트롤
...
class PacketContoroller {
	...
	constructor(buf: any)
	{
		if (Number.isInteger(buf)) {
			this.buf = Buffer.alloc(buf); // 해당 크기만큼 buf 할당
		} 
		else {
			this.buf = Buffer.from(buf); // 해당 데이터 buf에 할당
		}
	}
	...
	static depktize(buf: any)
	{
		...
		const rv = new PacketContoroller(buf.slice(0, theader.pkt_size())); // 총 저장 버퍼에서 해당 크기만큼 잘라서 새로운 버퍼 생성
		rv.parse(); // 해당 버퍼 파싱
		return rv; // 해당 파싱된 데이터 return
	}
